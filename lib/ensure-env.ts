...
for (const key of required) {
  if (!process.env[key]) {
    console.error(
      `[ENV VALIDATION] ❌ Missing required environment variable: ${key}`,
    );
    hasMissing = true;
  }
}
...
if (hasMissing) {
  console.error(
    '[ENV VALIDATION] Build failed due to missing environment variables. Please check deployment settings for the "Preview" environment.',
  );
  process.exit(1);
} else {
  console.log(
